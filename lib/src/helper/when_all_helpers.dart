...
Cont<E, F, A3> _bothWhenAll<E, F, A1, A2, A3>(
  ...
  return Cont.fromRun((runtime, observer) {
    final (handlePrimary, handleSecondary) = _whenAll<
        E,
        ContError<F>,
        ContError<F>,
        ContError<F>,
      ...
      onSecondary: (triple) {
        switch (triple) {
          case _Value1<ContError<F>, ContError<F>,
                ContError<F>>(a: final f):
          case _Value2<ContError<F>, ContError<F>,
                ContError<F>>(b: final f):
          case _Value3<ContError<F>, ContError<F>,
                ContError<F>>(c: final f):
            observer.onElse(f);
        }
      },
...
Cont<E, F3, A> _eitherWhenAll<E, F1, F2, F3, A>(
  ...
  return Cont.fromRun((runtime, observer) {
    final (handlePrimary, handleSecondary) = _whenAll<E, A,
        A, A, ContError<F1>, ContError<F2>, ContError<F3>>(
      runtime: runtime,
